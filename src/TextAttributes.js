 ...
 *     new DOM containers.
 */
const TextAttributes = function(document) {
...
TextAttributes.prototype.resetColorPalette = function() {
  this.colorPalette = colors.colorPalette.concat();
  this.syncColors();
};
...
TextAttributes.prototype.syncColors = function() {
  ...
  if (this.faint && !this.invisible) {
    ...
    this.foreground = colors.mix(colorToMakeFaint, 'rgb(0, 0, 0)', 0.3333);
  }
...
TextAttributes.nodeWidth = function(node) {
  if (node.wcNode) {
    return wc.strWidth(node.textContent);
  } else {
...
TextAttributes.nodeSubstr = function(node, start, width) {
  if (node.wcNode) {
    return wc.substr(node.textContent, start, width);
  } else {
...
TextAttributes.nodeSubstring = function(node, start, end) {
  if (node.wcNode) {
    return wc.substring(node.textContent, start, end);
  } else {
