...
const VT = function(terminal) {
  ...
  var cc1 = Object.keys(VT.CC1).map(
      function(e) {
        return '\\x' + f.zpad(e.charCodeAt().toString(16), 2)
      }).join('');
  this.cc1Pattern_ = new RegExp('[' + cc1 + ']');

  // Decoder to maintain UTF-8 decode state.
  this.utf8Decoder_ = new UTF8Decoder();
...
VT.prototype.onTerminalMouse_ = function(e) {
  ...
  var x = String.fromCharCode(f.clamp(e.terminalColumn + 32, 32, 255));
  var y = String.fromCharCode(f.clamp(e.terminalRow + 32, 32, 255));
...
VT.OSC['4'] = function(parseState) {
  ...
  for (var pairNumber = 0; pairNumber < pairCount; ++pairNumber) {
    ...
    if (colorValue == '?') {
      ...
      colorValue = colors.rgbToX11(colorPalette[colorIndex]);
      if (colorValue)
    ...
    colorValue = colors.x11ToCSS(colorValue);
    if (colorValue)
...
VT.OSC['50'] = function(parseState) {
  ...
  switch (args[1]) {
    case '1':
      this.terminal.setCursorShape(Terminal.cursorShape.BEAM);
      break;

    case '2':
      this.terminal.setCursorShape(Terminal.cursorShape.UNDERLINE);
      break;

    default:
      this.terminal.setCursorShape(Terminal.cursorShape.BLOCK);
  }
};
...
VT.CSI['I'] = function(parseState) {
  ...
  count = f.clamp(count, 1, this.terminal.screenSize.width);
