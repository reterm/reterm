...
import identifiers from './parser/identifiers'
...
Parser.prototype.reset = function(source, opt_pos) {
  this.source = source;
  this.pos = opt_pos || 0;
 ...
 * identifiers.modifierKeys followed by a key code.  Key
 * codes can be an integer or an identifier from
 * identifiers.keyCodes.  Modifiers and keyCodes should be joined
 ...
 *   properties.
 */
Parser.prototype.parseKeySequence = function() {
  var rv = {
    keyCode: null
  };

  for (var k in identifiers.modifierKeys) {
    rv[identifiers.modifierKeys[k]] = false;
  }
  ...
  while (this.pos < this.source.length) {
    ...
    if (token.type == 'integer') {
      ...
      if (token.value in identifiers.modifierKeys) {
        var mod = identifiers.modifierKeys[token.value];
        if (rv[mod] && rv[mod] != '*')
          throw this.error('Duplicate modifier: ' + token.value);
        rv[mod] = true;

      } else if (token.value in identifiers.keyCodes) {
        rv.keyCode = identifiers.keyCodes[token.value];
      ...
      if (token.value == '*') {
        for (var id in identifiers.modifierKeys) {
          var p = identifiers.modifierKeys[id];
          if (!rv[p])
            rv[p] =  '*';
        }
      } else {
...
Parser.prototype.parseKeyAction = function() {
  this.skipSpace();
  ...
  if (token.type == 'identifier') {
    if (token.value in identifiers.actions)
      return identifiers.actions[token.value];
...
Parser.prototype.peekString = function() {
  return this.ch == '\'' || this.ch == '"';
};

Parser.prototype.peekIdentifier = function() {
  return this.ch.match(/[a-z_]/i);
};

Parser.prototype.peekInteger = function() {
 ...
 * @param {number} count
 */
Parser.prototype.advance = function(count) {
  this.pos += count;
 ...
 * @return {void}
 */
Parser.prototype.skipSpace = function(opt_expect) {
  if (!/\s/.test(this.ch))
    return;
