...
import wc from 'reterm-libdot/lib/wc';
...
import { windowType, desktopNotificationTitle, getClientSize } from './hterm'

/**
 * Constructor for the Terminal class.
 *
 * A Terminal pulls together the ScrollPort, Screen and VT100
 ...
 */
const Terminal = function(opt_profileId) {
  ...
  this.cursorShape_ = Terminal.cursorShape.BLOCK;
 ...
 */
Terminal.prototype.onTerminalReady = function() { };
 ...
 */
Terminal.prototype.setProfile = function(profileId, opt_callback) {
  ...
  this.prefs_ = new PreferenceManager(this.profileId_);
  this.prefs_.addObservers(null, {
    ...
    'character-map-overrides': function(v) {
      ...
      for (var code in v) {
        var glmap = VT.CharacterMap.maps[code].glmap;
        for (var received in v[code]) {
          glmap[received] = v[code][received];
        }
        VT.CharacterMap.maps[code].reset(glmap);
      }
    },
    ...
    'color-palette-overrides': function(v) {
      ...
      colors.colorPalette = colors.stockColorPalette.concat();
      ...
      if (v) {
        for (var key in v) {
          ...
          if (v[i]) {
            var rgb = colors.normalizeCSS(v[i]);
            if (rgb)
              colors.colorPalette[i] = rgb;
          }
        }
      }
...
Terminal.prototype.setBackgroundColor = function(color) {
  this.backgroundColor_ = colors.normalizeCSS(color);
...
Terminal.prototype.setForegroundColor = function(color) {
  this.foregroundColor_ = colors.normalizeCSS(color);
  this.primaryScreen_.textAttributes.setDefaults(
      this.foregroundColor_, this.backgroundColor_);
  this.alternateScreen_.textAttributes.setDefaults(
      this.foregroundColor_, this.backgroundColor_);
 ...
 */
Terminal.prototype.runCommandClass = function(commandClass, argString) {
 ...
 */
Terminal.prototype.syncFontFamily = function() {
 ...
 * necessary.
 */
Terminal.prototype.syncBoldSafeState = function() {
 ...
 *
 * @return {RowCol} The RowCol object representing the current position.
 */
 ...
 *
 * @param {RowCol} cursor The position to restore.
 */
Terminal.prototype.restoreCursor = function(cursor) {
  var row = f.clamp(cursor.row, 0, this.screenSize.height - 1);
  var column = f.clamp(cursor.column, 0, this.screenSize.width - 1);
...
Terminal.prototype.print = function(str) {
  ...
  while (startOffset < strWidth) {
    ...
    if (didOverflow && !this.options_.wraparound) {
      ...
      substr = wc.substr(str, startOffset, count - 1) +
          wc.substr(str, strWidth - 1);
      count = strWidth;
    } else {
      substr = wc.substr(str, startOffset, count);
    }
...
Terminal.prototype.ringBell = function() {
  ...
  if (this.desktopNotificationBell_ && !this.document_.hasFocus()) {
    var n = new Notification(
        f.replaceVars(desktopNotificationTitle,
...
Terminal.prototype.restyleCursor_ = function() {
  ...
  switch (shape) {
    case Terminal.cursorShape.BEAM:
    ...
    case Terminal.cursorShape.UNDERLINE:
...
Terminal.prototype.showOverlay = function(msg, opt_timeout) {
  ...
  var divSize = getClientSize(this.div_);
  var overlaySize = getClientSize(this.overlayNode_);
...
Terminal.prototype.copyStringToClipboard = function(str) {
  if (this.prefs_.get('enable-clipboard-notice'))
    setTimeout(this.showOverlay.bind(this, notifyCopyMessage, 500), 200);
  ...
  copySelectionToClipboard(this.document_);
...
Terminal.prototype.getSelectionText = function() {
  ...
  return wc.substring(rv, startOffset, wc.strWidth(rv) - endOffset);
};
...
Terminal.prototype.onResize_ = function() {
  ...
  var rowCount = f.smartFloorDivide(this.scrollPort_.getScreenHeight(),
 ...
 */
Terminal.prototype.setScrollWheelMoveMultipler = function(multiplier) {
