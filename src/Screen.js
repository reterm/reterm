 ...
 *
 * In practice...
 * - The Terminal class holds two Screen instances.  One for the
 ...
 * ScrollPort and have to follow a few rules as a result.  Each
 ...
 * of the scrollback buffer.  These invariants are enforced by Terminal
 * because that is the class using the Screen in the context of an
 * ScrollPort.
 */
 ...
 *    0 if not provided.
 */
const Screen = function(opt_columnCount) {
  /**
  ...
  this.cursorPosition = new RowCol(0, 0);
...
/**
 * Return the screen size as an Size object.
 *
 * @return {Size} Size object representing the current number
 *     of rows and columns in this screen.
 */
Screen.prototype.getSize = function() {
  return new Size(this.columnCount_, this.rowsArray.length);
};
...
Screen.prototype.clearCursorRow = function() {
  ...
  if (this.textAttributes.isDefault()) {
    ...
    text = f.getWhitespace(this.columnCount_);
  }
...
Screen.prototype.splitNode_ = function(node, offset) {
  ...
  afterNode.textContent = wc.substr(textContent, offset);
...
Screen.prototype.insertString = function(str) {
  ...
  if (reverseOffset < 0) {
    ...
    // whitespace.
    var ws = f.getWhitespace(-reverseOffset);
  ...
  if (offset == 0) {
    ...
    if (previousSibling &&
      ...
      this.cursorOffset_ = wc.strWidth(previousSibling.textContent);
      return;
    }
  ...
  if (reverseOffset == 0) {
    ...
    if (nextSibling &&
      ...
      this.cursorOffset_ = wc.strWidth(str);
      return;
    }
...
Screen.prototype.expandSelection = function(selection) {
  ...
  var lineUpToRange = wc.substring(rowText, 0, endPosition);
